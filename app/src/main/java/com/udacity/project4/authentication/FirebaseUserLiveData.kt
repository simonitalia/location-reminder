...
class FirebaseUserLiveData : LiveData<FirebaseUser?>() {
    ...
    // set value of FireUserLiveData object by hooking up to value of current FirebaseUser
    private val authStateListener = FirebaseAuth.AuthStateListener { firebaseAuth ->
        value = firebaseAuth.currentUser
    }

    // When object has active observer, start observing FirebaseAuth state to check if
    // there is currently a logged in user
    override fun onActive() {
        firebaseAuth.addAuthStateListener(authStateListener)
    }

    // When object no longer has an active observer, stop observing the FirebaseAuth state to
